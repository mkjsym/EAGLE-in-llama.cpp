...
static struct ggml_tensor * llm_build_inp_embd_eagle(
    struct ggml_context * ctx,        // GGML 컨텍스트
    struct llama_context & lctx,      // LLaMA 컨텍스트 (입력 텐서 저장 등)
    const llama_hparams & hparams,    // 모델 하이퍼파라미터 (n_embd 등)
    const llama_ubatch & ubatch,      // 처리할 사용자 배치 정보 (토큰 또는 임베딩)
    ...
    struct llama_context & ctx_tgt) {       // 디버깅/로깅 콜백 함수
...
/*static struct ggml_tensor * llm_build_fc(
    ...
    // 입력 차원 확인
    int64_t input_dim = input->ne[0];
    if (input_dim <= 0) {
        GGML_ABORT("Input dimension must be greater than 0");
    }

    // 출력 차원 계산 (입력 차원의 절반)
    int64_t output_dim = input_dim / 2;
    
    if (weights->ne[0] != output_dim || weights->ne[1] != input_dim) {
        GGML_ABORT("Weight dimension mismatch");
    }
    ...
    // bias가 있으면 더함
    if (bias) {
        if (bias->ne[0] != output_dim) {
            GGML_ABORT("Bias dimension mismatch");
        }
...
struct llm_build_context {
    ...
    // struct ggml_cgraph * append_lmhead() {
    ...
    void append_embedding_layer(struct ggml_cgraph * gf, struct ggml_tensor * cur) {
    ...
    struct ggml_cgraph * build_eagle(struct llm_build_context & llm2) {
        struct ggml_cgraph * gf = ggml_new_graph_custom(ctx0, model.max_nodes(), false);
...
static struct ggml_cgraph * llama_build_graph_eagle(
    llama_context & lctx,
...
struct llm_build_context llm(lctx, ubatch, cb, worst_case);
struct llm_build_context llm2(ctx_tgt, ubatch, cb, worst_case);
...
while (lctx.sbatch.n_tokens > 0) {
    ...
    // llama_build_graph: 현재 ubatch를 처리하기 위한 GGML 계산 그래프 생성.
    // false 인자는 프롬프트 인코딩용이 아님을 의미할 수 있음.
    ggml_cgraph * gf = llama_build_graph_eagle(lctx, ctx_tgt, ubatch, false);
            ...
            case LLAMA_POOLING_TYPE_NONE: // 토큰별 임베딩
                {
                    ...
                    if (n_outputs_new) {
                        ...
                        // 비동기 복사 시작
                        ggml_backend_tensor_get_async(backend_embd, embd, embd_out, 0, n_outputs_new*n_embd*sizeof(float));
                        //ggml_backend_tensor_get_async(backend_embd, embd, lctx.hidden, 0, n_outputs_new*n_embd*sizeof(float));
                    }
                } break;
            case LLAMA_POOLING_TYPE_MEAN: // 시퀀스 평균 풀링
            case LLAMA_POOLING_TYPE_CLS:  // CLS 토큰 풀링
...
const int64_t n_tokens = inp_batch.n_tokens;

/////////////////////////////////////////////////////////////
size_t num_elements = (size_t)n_tokens * n_embd;
float * src_ptr = (float*)lctx.embd;

if (n_tokens > 0 && n_embd > 0) {
    float* check_ptr = (float*)lctx.embd;
    printf("DEBUG: Checking source data just before copy @ %p\n", (void*)check_ptr);
    printf("DEBUG: Source [0]=%f, [1]=%f, [2]=%f\n", check_ptr[0], check_ptr[1], check_ptr[2]);
    fflush(stdout);

    // ... 여기에 데이터 복사 코드 ...
}

std::vector<float> copied_embd_data(src_ptr, src_ptr + num_elements);
/////////////////////////////////////////////////////////////

if (!copied_embd_data.empty()) {
    printf("  Copied Data [Token 0 Start]:");
    // 벡터 크기와 3 중 작은 값만큼만 출력 (벡터가 3개 미만일 수 있음)
    for (size_t i = 0; i < std::min((size_t)3, copied_embd_data.size()); ++i) {
        printf(" %f", copied_embd_data[i]); // 벡터 요소 접근은 [] 사용
    }
    printf("\n");

    // 마지막 토큰의 마지막 3개 요소 출력 (충분한 요소가 있을 경우)
    if (copied_embd_data.size() >= 3) { // 최소 3개 요소가 있는지 확인
         printf("  Copied Data [Last 3 Elements]:");
         size_t last_elem_start_idx = copied_embd_data.size() - 3;
         for (size_t i = 0; i < 3; ++i) {
              printf(" %f", copied_embd_data[last_elem_start_idx + i]);
         }
         printf("\n");
    }
}
...
if (n_tokens > 0) {
    printf("5555555555Draft Hidden State (saved to ctx_dft->hidden) @ %p:\n", (void*)lctx.embd);
    // 첫 번째 토큰의 처음 3개 값 출력
...
lctx.embd = copied_embd_data.data();
if (n_tokens > 0) {
    printf("111111111111111111Draft Hidden State (saved to ctx_dft->hidden) @ %p:\n", (void*)lctx.embd);
    // 첫 번째 토큰의 처음 3개 값 출력
    printf("  [Token 0 Start]: %f, %f, %f\n",
        ((float*)lctx.embd)[0],
        ((float*)lctx.embd)[1],
...
if (n_tokens > 0) {
    printf("222222222222222222Draft Hidden State (saved to ctx_dft->hidden) @ %p:\n", (void*)lctx.embd);
    // 첫 번째 토큰의 처음 3개 값 출력
    printf("  [Token 0 Start]: %f, %f, %f\n",
        ((float*)lctx.embd)[0],
        ((float*)lctx.embd)[1],
            ...
            case LLAMA_POOLING_TYPE_NONE: // 토큰별 임베딩
                {
                    ...
                    if (embd_out != nullptr) { // embd_out은 ctx_dft->hidden을 가리킴
                        ...
                        if (n_outputs_new > 0) {
                            printf("3333333333Draft Hidden State (saved to ctx_dft->hidden) @ %p:\n", (void*)embd_out);
                            // 첫 번째 토큰의 처음 3개 값 출력
                            printf("  [Token 0 Start]: %f, %f, %f\n",
                                ((float*)embd_out)[0],
                                ((float*)embd_out)[1],
            ...
            case LLAMA_POOLING_TYPE_NONE: // 토큰별 임베딩
                {
                    ...
                    //fflush(stdout); // 버퍼를 비워 즉시 출력되도록 함
                    // float * embd_out = lctx.embd + n_outputs_prev*n_embd; // 출력 버퍼 내 위치 계산
                    float * embd_out = ctx_dft.embd + n_outputs_prev*n_embd; // 출력 버퍼 내 위치 계산
                    const int32_t n_outputs_new = lctx.n_outputs;
                    //printf("embedding 추출1-1\n\n");
                    if (n_outputs_new) {
                        // ... (Assertions) ...
                    ...
                    // printf("tensor set start %f\n\n", *embd_out); // 출력
                    // fflush(stdout); // 버퍼를 비워 즉시 출력되도록 함
                    // //ctx_dft.inp_hidd = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd, ubatch.n_tokens);
...
while (lctx.sbatch.n_tokens > 0) {
   ...
   llama_set_inputs_initial(lctx, ubatch);
...
int32_t llama_decode_draft(
    ...
    struct llama_batch   batch,
    struct llama_context * ctx_tgt) {
const int ret = llama_decode_draft_impl(*ctx, batch, *ctx_tgt);
if (ret != 0) {
    LLAMA_LOG_ERROR("%s: failed to decode, ret = %d\n", __func__, ret);
}
