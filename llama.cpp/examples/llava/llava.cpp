...
struct llava_embd_batch {
    ...
    llama_batch batch;
    llava_embd_batch(float * embd, float * hidd, int32_t n_tokens, llama_pos pos_0, llama_seq_id seq_id) {
        pos     .resize(n_tokens);
...
bool llava_eval_image_embed(llama_context * ctx_llama, const struct llava_image_embed * image_embed, int n_batch, int * n_past) {
    ...
    for (int i = 0; i < image_embed->n_image_pos; i += n_batch) {
        ...
        float * hidd = image_embed->embed+i*n_embd;
        llava_embd_batch llava_batch = llava_embd_batch(embd, hidd, n_eval, *n_past, 0);
